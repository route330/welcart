...
* Fixed the bug that displaying gSecurity check4h when reload the page after login to my page.
* Fixed the bug that displaying gSecurity check2h in case without wc_templetes when click the button of gNext with membership for registerh.
* Fixed the bug that the item registration number of Welcart Shop Home doesnft decrease when the item delete(into the trash box). 
* Fixed the bug that displaying gNoticeh on custom order field check.
