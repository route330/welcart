...
//商品合計計算部分（海外通貨で少数有りの場合、有効桁数（2桁）決めて丸める）
		...
		recalculation : function() {
			...
			var s = orderfunc.settings;
			s.url = uscesL10n.requestFile;
			s.data = 'action=order_item_ajax&mode=recalculation&order_id='+$('#order_id').val()+'&mem_id='+$('#member_id_label').html()+'&post_ids='+post_ids+'&skus='+skus+'&prices='+prices+'&quants='+quants+'&upoint='+order_usedpoint+'&shipping_charge='+order_shipping_charge+'&cod_fee='+order_cod_fee+'&discount='+order_discount;
			s.success = function(data, dataType) {
				var values = data.split('#usces#');
				if( 'ok' == $.trim(values[0]) ) {
